...
void loop() {
  ...
  for(int nivel = 1; nivel < LEVELS; nivel++){
    if (lost) break;
    Serial.print("GLOBAL "); Serial.println(nivel);
    for(int i = 0; i <= nivel;){
      int delay_count = 0;
      if (lost) break;
      Serial.print("Nivel "); Serial.println(i);
      ledsOn(secuencia[i]);
      delay(1000);
      ledsOff();
      Serial.print("      BOTON : "); Serial.println(buttonPress());
      while(buttonPress() == -1){
        delay(1);
        delay_count++;
      ...
      (buttonPress()!=secuencia[i]) ? lost = 1 : i++;
    }
  }
  if(lost){
    looseSong();
